...
问题描述：在 Android 15 系统下，相机拍照后的临时文件写入与 Flutter 的读取存在微秒级的时差，直接调用会导致 FileNotfound 或识别引擎卡死。
...
问题描述：Flutter 的 google_ml_kit 插件在 Android 原生层默认仅包含拉丁语（Latin）脚本，导致中文识别时抛出 ClassNotFoundException 导致闪退。
